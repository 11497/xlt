...
- 结合对话历史重写问题
- 会话标题自动总结
- 恶意内容检测（被拦截输入和固定拒绝回复落库并打标记，不参与后续问题重写与生成）
- 生成后来源相关性判定，保存当时使用的切片正文快照，并支持在聊天页和会话详情中查看
...
| `session`                 | 会话                  |
| `message`                 | 消息                  |
| `source_content`          | 回答来源正文快照       |
| `message_source`          | 回答来源引用快照       |
| `announcement`            | 公告                  |
| `announcement_attachment` | 公告附件              |

除 `document_task` 任务状态表和按内容去重的不可变正文表 `source_content` 外，业务表均包含 `is_deleted` 和 `deleted_at` 字段。普通业务查询默认过滤已删除记录；数据库外键统一使用 `RESTRICT`，避免隐性级联硬删除。已逻辑删除的用户、角色、知识库及其关联不参与权限计算，旧 JWT 不能继续访问。
...
ChromaDB 与 Elasticsearch 的切片 ID 使用相同的 `{document_id}_{chunk_index}` 格式（即 `document_id_chunk_index`），混合检索按该 ID 融合去重。完整回答生成后，utility 模型会根据用户问题、完整回答和带 ID 的候选切片判定实际使用的切片；`message_source` 与 assistant 消息在同一事务落库。这些表保存当时的正文和文件名快照，不是 ChromaDB/ES 的索引镜像，重索引或文档逻辑删除后历史消息仍返回当时快照。
