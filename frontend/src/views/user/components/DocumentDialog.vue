...
import { deleteDocument, downloadDocument, getDocumentListByKnowledgeBase, uploadDocument } from '@/api/document.js';
...
const handleUploadRequest = async (options) => {
  ...
  try {
    ...
    if (res.code === 1) { ElMessage.success('上传成功'); options.onSuccess?.(res); await fetchDocuments(); }
  ...
  <el-dialog v-model="dialogVisible" :title="`知识库文档列表 - ${currentKbName}`" width="900px" destroy-on-close @closed="resetState">
    ...
    <el-table :data="documentList" border style="width: 100%" v-loading="loading" empty-text="暂无文档">
      ...
      <el-table-column label="操作" width="180" align="center" fixed="right"><template #default="{ row }">
        ...
        <el-button v-if="canManage()" type="danger" link @click="handleDelete(row)"><el-icon><Delete /></el-icon> 删除</el-button>
