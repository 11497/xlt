...
- **响应式布局**: 支持桌面端和移动端，移动端导航与会话列表使用抽屉交互
- **界面系统**: 采用统一的校园知识工作台视觉变量、Element Plus 主题和共享布局组件
...
- 角色-知识库权限：`0` 为只读，`1` 为读写；用户拥有多个角色时取权限最大值
- 普通用户可查看、下载、查询和聊天检索其有只读或读写权限的知识库，只有读写权限允许上传和删除文档
- 管理员拥有全部知识库权限；知识库创建、改名和删除仅限管理员
...
### 6. 管理概览
- 汇总展示用户、角色、知识库、会话和公告数量
- 展示最近发布的公告，并支持跳转到对应管理页面

### 7. 校园知识工作台
- 用户端、管理端和聊天页共用统一的顶部导航；用户端与管理端共用侧边导航和页面标题层级，聊天页使用独立的会话侧栏
- 登录页使用本地 AI 生成的校园图书馆视觉资产，提供输入图标、密码可见和登录加载状态
- 登录页、顶部导航和浏览器标签统一使用本地 SVG 品牌图标 `frontend/src/assets/xlt-icon.svg`
- 个人工作台集中展示账号身份、置顶公告、常用入口和密码修改操作
- 聊天页提供会话选中状态、身份标识、流式生成反馈、固定输入区及优化后的 Markdown 阅读样式
- 管理列表统一操作栏、状态标签、空状态、表格、分页和弹窗；移动端保留抽屉导航并适配小屏触控尺寸
...
其中，`001` 为角色-知识库关联表增加权限字段，`002` 清理历史会话名称并将字段约束收紧到 1–30 个字符。AI 自动生成的会话标题仍以 2–20 个字符为目标。
...
- 密码：6–20 个字符
- 角色名、知识库名：1–15 个字符
- 会话名：1–30 个字符（AI 自动标题仍要求 2–20 个字符）
