...
class AnnouncementCRUD:
    ...
    def update_update_time(announcement_id: int) -> bool:
        ...
        sql = "UPDATE announcement SET update_time = CURRENT_TIMESTAMP WHERE id = %s AND is_deleted = 0"
        with get_cursor() as cursor:
    ...
    def update(announcement_id: int, title: str, content: str, is_top: int) -> bool:
        ...
        sql = "UPDATE announcement SET title = %s, content = %s, is_top = %s WHERE id = %s AND is_deleted = 0"
        with get_cursor() as cursor:
    ...
    def batch_delete(announcement_ids: List[int]) -> int:
        ...
        sql = f"UPDATE announcement SET is_deleted = 1, deleted_at = NOW(), update_time = NOW() WHERE is_deleted = 0 AND id IN ({placeholders})"
        with get_cursor() as cursor:
    ...
    def get_by_id(announcement_id: int) -> Optional[Announcement]:
        ...
        :return: 公告对象（如果存在）
        """
        sql = "SELECT * FROM announcement WHERE id = %s AND is_deleted = 0"
        with get_cursor() as cursor:
    ...
    def get_all() -> List[Announcement]:
        ...
        :return: 公告对象列表
        """
        sql = "SELECT * FROM announcement WHERE is_deleted = 0 ORDER BY is_top DESC, create_time DESC"
        with get_cursor() as cursor:
            cursor.execute(sql)
    ...
    def get_page(page: int = 1, page_size: int = 10) -> Tuple[List[Announcement], int]:
        ...
        sql_count = "SELECT COUNT(*) AS total FROM announcement WHERE is_deleted = 0"
        sql_data = "SELECT * FROM announcement WHERE is_deleted = 0 ORDER BY is_top DESC, create_time DESC LIMIT %s OFFSET %s"
    ...
    def get_recent(limit: int = 5) -> Tuple[List[Announcement], int]:
        """查询最近发布的公告及公告总数。"""
        sql_count = "SELECT COUNT(*) AS total FROM announcement WHERE is_deleted = 0"
        sql_data = "SELECT * FROM announcement WHERE is_deleted = 0 ORDER BY create_time DESC LIMIT %s"
