...
@router.post("/assign")
async def batch_assign_role_to_knowledge_base(knowledge_base_id: int, role_ids: List[int],
...
async def batch_remove_roles_from_knowledge_base(knowledge_base_id: int, role_ids: List[int],
                                               _admin: User = Depends(require_admin)):
    """批量从指定知识库中删除角色"""
    result = Result()
...
async def get_roles_by_knowledge_base(knowledge_base_id: int, _admin: User = Depends(require_admin)):
    """获取指定知识库的所有角色ID"""
    result = Result()
...
async def get_knowledge_base_by_role(role_id: int, _admin: User = Depends(require_admin)):
    """获取指定角色的所有知识库ID"""
    result = Result()
...
async def assign_knowledge_base_to_role(role_id: int, knowledge_base_id: int, _admin: User = Depends(require_admin)):
    """分配单个知识库到指定角色"""
    result = Result()
...
async def remove_knowledge_base_from_role(role_id: int, knowledge_base_id: int, _admin: User = Depends(require_admin)):
    """从指定角色中移除单个知识库"""
    result = Result()
...
async def delete_by_knowledge_base(knowledge_base_id: int, _admin: User = Depends(require_admin)):
    """删除指定知识库的所有角色关联关系"""
    result = Result()
